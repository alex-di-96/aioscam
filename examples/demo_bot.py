...
async def cmd_help(event):
    ...
    help_text = (
        ...
        f"• Inline клавиатуры (one_time поведение)\n\n"
        f"📝 **FSM-флоу (кнопки в /start):**\n"
        f"• 📝 Регистрация — 4 шага: имя → возраст → email → телефон\n"
        f"• 🎯 Викторина — 3 вопроса с inline A/B/C/D кнопками\n"
        f"• 💬 Обратная связь — рейтинг 1–5 + текстовый комментарий\n"
        f"• 🖼️ Изображение — отражение + 800×600 + водяной знак AioScam\n\n"
        f"🎛️ **Архитектура:**\n"
        f"• Router + Middleware\n"
        f"• Magic filters (F.text, F.callback)\n"
        f"• StateGuard (блокировка команд во время FSM)\n"
        f"• Polling + Webhook (FastAPI, Litestar)\n"
        f"• Rate Limiter (token bucket, 429 retry)\n\n"
        f"📦 **API:** 35 методов | **События:** 14 типов\n"
        f"🖼️ **Медиа:** фото, видео, аудио, документы\n"
        f"━━━━━━━━━━━━━━━━━━\n\n"
        f"**Команды:**\n"
        f"/start — главное меню\n"
        f"/help — эта справка\n"
        f"/stats — статистика + HTML демо\n"
        f"/register — регистрация (4 шага: имя, возраст, email, телефон)\n"
        f"/feedback — обратная связь\n"
        f"/cancel — отмена текущей операции\n"
        f"/contact — демо запроса контакта\n"
        f"/location — демо запроса геолокации\n"
...
async def cmd_stats(event):
    ...
    stats_text = (
        ...
        f"🧪 <b>Тестов:</b> 202/202 passed (100%)\n"
        f"🔒 <b>Security Score:</b> 9/10\n\n"
        f"<b>Реализовано:</b>\n"
        f"• 35+ API методов Max\n"
        f"• 14 типов событий\n"
        f"• 8 типов кнопок (inline)\n"
        f"• Медиа: фото, видео, аудио, документы\n"
        f"• Загрузка файлов (upload + download)\n"
        f"• FSM с storage backends\n"
        f"• Middleware система\n"
        f"• Router система (вложенные роутеры)\n"
        f"• Magic filters\n"
        f"• StateGuard\n"
        f"• Deep links + StartCommand фильтр\n"
        f"• I18n (JSON, авто-локаль)\n"
        f"• Rate Limiter (token bucket)\n"
        f"• Запрос контакта/геолокации\n"
        f"• Webhook (aiohttp / FastAPI / Litestar)\n\n"
